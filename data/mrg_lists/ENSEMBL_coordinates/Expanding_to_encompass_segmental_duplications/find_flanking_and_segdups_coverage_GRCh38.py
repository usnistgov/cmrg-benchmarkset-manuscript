...
parser = argparse.ArgumentParser(description="Find flanking segmental duplications")
...
for gene in f_input_genes_lines:
    ...
    tmf.flush()
    tmf.close()
    subprocess.check_output("bedtools intersect -a GRCh38_segdups.bed.gz -b temp_gene_file_" + gene_name + ".bed -wa | multiIntersectBed -i stdin temp_gene_file_" + gene_name + ".bed | sed 's/^chr//' | sort -k1,1 -k2,2n | sed 's/^/chr/' | bedtools merge -i stdin > temp_gene_file_" + gene_name + "_with_flanking_sequence_segdups.bed", shell = True)
