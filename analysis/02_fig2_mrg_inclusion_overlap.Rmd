...
diff_overlap_df <- bench_cov_anno_df %>%
    ...
    mutate(gene_cat = factor(
        gene_cat,
        level = c("V4", "MRG", "Excluded"),
        labels = c(">90% coverage by V4.2.1",
                   ">90% coverage by MRG Benchmark",
                   "Excluded")
    ))

(diff_overlap_plt <- ggplot(diff_overlap_df) + 
...
diff_overlap_df <- bench_cov_anno_df %>%
    ...
    mutate(gene_cat = factor(
        ...
        labels = c("Excluded",
                   ">90% coverage by V4.2.1",
                   ">90% coverage by MRG Benchmark")
    ))

diff_overlap_cnts <- diff_overlap_df %>% 
    mutate(threshold = if_else(overlap > 0.97, "full","partial")) %>% 
    count(gene_cat, threshold) %>% 
    pivot_wider(id_cols = "gene_cat", names_from = "threshold", values_from = "n") %>% 
    mutate(full_prop = round(full/ (full + partial), 2),
           anno_text = paste0(full_prop, " (",full,")")) %>% 
    add_column(y = c(0.9, 0.5, 0.1))
...
(diff_overlap_plt <- ggplot(diff_overlap_df) + 
    ...
    geom_text(data = diff_overlap_cnts, 
              aes(x = 0.95, y = y, label = anno_text)) + 
    # facet_wrap(~gene_cat, ncol = 1, scales = "free_y") + 
    theme_bw() + 
...
```{r}
(diff_overlap_plt <- ggplot(diff_overlap_df) + 
    stat_ecdf(aes(x = 1 - overlap, fill = gene_cat), 
              alpha = 0.25, geom = "area") + 
         stat_ecdf(aes(x = 1-overlap, color = gene_cat), 
                   geom = "line") + 
    # facet_wrap(~gene_cat, ncol = 1, scales = "free_y") + 
    ...
    theme_bw() + 
    theme(legend.position = "bottom") + 
    labs(x = "Fraction of gene NO OVERLAP challenging sequence or variants", 
         y = "Fraction of Genes",
         color = "Benchmark",
         fill = "Benchmark")) #+ 
    # coord_flip()
...
```{r}
(diff_overlap_plt <- ggplot(diff_overlap_df) + 
    ggdist::stat_ccdfinterval(aes(x = overlap, y = gene_cat, fill = gene_cat), 
              alpha = 0.25, show_interval = FALSE) + 
    # facet_wrap(~gene_cat, ncol = 1, scales = "free_y") + 
...
ggplot(cum_rank_df) +
    geom_line(aes(y = gene_rank_frac * 100, x = overlap, color = gene_cat)) +
    geom_vline(aes(xintercept = 0.15), linetype = 2) +
    geom_point(data = cum_rank_text_df,
               aes(x = overlap, y = gene_rank_frac * 100, fill = gene_cat),
               shape = 21) +
    # ggforce::geom_mark_rect(data = cum_rank_text_df,
    #            aes(x = overlap,
    #                y = gene_rank_frac,
    #                group = gene_cat,
    #                label = gene_cat,
    #                description = paste0(round(gene_rank_frac, 2),
    #                               " (", n_genes_gt15_overlap,")")),
    #            color = "transparent", label.buffer = unit(25, 'mm'),
    #            label.fill = "transparent") +
    # ggrepel::geom_label_repel(data = cum_rank_text_df,
    #                           aes(
    #                               x = overlap,
    #                               y = gene_rank_frac,
    #                               group = gene_cat,
    #                               # label = gene_cat,
    #                               label = paste0(round(gene_rank_frac, 2),
    #                                              " (", n_genes_gt15_overlap, ")")
    #                           )) +
    geom_text(data = cum_rank_text_df, 
              aes(x = overlap, 
    ...
    labs(x = "Fraction of gene region with challenging sequence or variants",
         y = "% of Genes with > X Challenging", color = "", fill = "") 
```
...
ggplot(cum_rank_df) +
    geom_segment(data = cum_rank_text_df,
               aes(x = overlap_round, xend = overlap_round, y = 0, yend = gene_rank_frac * 100),
               linetype = 2) +
    geom_ribbon(aes(ymin = 0, ymax = gene_rank_frac * 100, 
                    x = overlap, 
                    fill = gene_cat),
                alpha = 0.25) +
        geom_line(aes(y = gene_rank_frac * 100, x = overlap, color = gene_cat)) +
